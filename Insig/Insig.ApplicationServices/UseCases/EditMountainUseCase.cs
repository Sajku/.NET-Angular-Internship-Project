...
{
    public class EditMountainUseCase : ICommandHandler<EditMountainCommand>
    {
        private readonly IMountainRepository _mountainRepository;
        private readonly IUnitOfWork _unitOfWork;

        public EditMountainUseCase(IMountainRepository mountainRepository, IUnitOfWork unitOfWork)
        {
            _mountainRepository = mountainRepository;
            _unitOfWork = unitOfWork;
        }

        public async Task Handle(EditMountainCommand command)
        {
            Mountain tempMountain = new Mountain(command.Id, command.Name, command.Height,
